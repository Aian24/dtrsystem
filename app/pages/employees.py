    ...
    with ui.dialog() as dlg, ui.card().style("width: 650px; max-width: 95vw; max-height: 90vh; overflow-y: auto;").classes("p-6"):
        ...
        with ui.card().classes("w-full mb-6 p-4 bg-slate-50 border border-slate-200 shadow-none"):
            with ui.element("div").classes("grid grid-cols-3 gap-3 w-full"):
                def_s1 = search_select(TIME_OPTIONS, value=default_state.get("work_start"), label="Work Start").props('outlined dense clearable options-dense popup-content-class="h-48 overflow-y-auto"')
                def_s2 = search_select(TIME_OPTIONS, value=default_state.get("break_out_1"), label="1st Break Out").props('outlined dense clearable options-dense popup-content-class="h-48 overflow-y-auto"')
                def_s3 = search_select(TIME_OPTIONS, value=default_state.get("break_in_1"), label="1st Break In").props('outlined dense clearable options-dense popup-content-class="h-48 overflow-y-auto"')
                def_s4 = search_select(TIME_OPTIONS, value=default_state.get("break_out_2"), label="2nd Break Out").props('outlined dense clearable options-dense popup-content-class="h-48 overflow-y-auto"')
                def_s5 = search_select(TIME_OPTIONS, value=default_state.get("break_in_2"), label="2nd Break In").props('outlined dense clearable options-dense popup-content-class="h-48 overflow-y-auto"')
                def_s6 = search_select(TIME_OPTIONS, value=default_state.get("work_end"), label="Work End").props('outlined dense clearable options-dense popup-content-class="h-48 overflow-y-auto"')
            ...
            with ui.card().classes("w-full mb-3 p-3 bg-white border border-slate-200 shadow-none"):
                ...
                with ui.column().classes("w-full mt-3").bind_visibility_from(enabled, "value"):
                    with ui.element("div").classes("grid grid-cols-3 gap-3 w-full").bind_visibility_from(is_rest, "value", value=False):
                        ws = search_select(TIME_OPTIONS, value=day_data.get("work_start"), label="Work Start").props('outlined dense clearable options-dense popup-content-class="h-48 overflow-y-auto"')
                        bo1 = search_select(TIME_OPTIONS, value=day_data.get("break_out_1"), label="1st Break Out").props('outlined dense clearable options-dense popup-content-class="h-48 overflow-y-auto"')
                        bi1 = search_select(TIME_OPTIONS, value=day_data.get("break_in_1"), label="1st Break In").props('outlined dense clearable options-dense popup-content-class="h-48 overflow-y-auto"')
                        bo2 = search_select(TIME_OPTIONS, value=day_data.get("break_out_2"), label="2nd Break Out").props('outlined dense clearable options-dense popup-content-class="h-48 overflow-y-auto"')
                        bi2 = search_select(TIME_OPTIONS, value=day_data.get("break_in_2"), label="2nd Break In").props('outlined dense clearable options-dense popup-content-class="h-48 overflow-y-auto"')
                        we = search_select(TIME_OPTIONS, value=day_data.get("work_end"), label="Work End").props('outlined dense clearable options-dense popup-content-class="h-48 overflow-y-auto"')
    ...
    with ui.dialog().classes('backdrop-blur-sm') as dialog:
        ...
        with ui.card().style("width: 450px; max-width: 90vw; padding: 24px; border-radius: 16px;"):
            ...
            with ui.row().classes("w-full justify-end"):
                ...
                ui.button("Delete Permanently", on_click=lambda: [dialog.close(), delete_employees(emp_ids, on_success)]).classes("btn btn-danger")
    dialog.open()
...
def open_edit_dialog(emp: dict, on_success):
    ...
    def content(dialog):
        ...
        with ui.element("div").classes("grid-cols-2").style("margin-top:14px;"):
            ...
            form["schedule"] = search_select(
                {"Mon-Sat": "Mon - Sat", "Mon-Fri": "Mon - Fri"}, 
                value=emp.get("schedule_type", "Mon-Sat"), 
                label="Work Schedule *"
            ).props("outlined dense options-dense").style("width:100%;")

        sched_dlg = render_custom_schedule_dialog(custom_sched_state, default_state, emp_name=emp.get("name", "Employee"))
...
def employees_page():
    ...
    def history_container():
        ...
        unique_depts = sorted(list(set(e["department"] for e in emps if e["department"] and e["department"] != "—")))
        dept_options = {"all": "— All Departments —", **{d: d for d in unique_depts}}
        sched_options = {"all": "— All Schedules —", "Mon-Sat": "Mon - Sat", "Mon-Fri": "Mon - Fri"}
            ...
            def on_submit(dialog):
                ...
                try:
                    ...
                    if not company_id_val:
                        toast_error("Validation Error", "Please select a company.")
                        return
        ...
        with ui.element("div").classes("card"):
            ...
            with ui.element("div").style(
                ...
                with ui.element("div").style("display: flex; align-items: center; gap: 10px; flex-wrap: wrap; flex: 1;"):
                    ...
                    # Company filter dropdown (searchable)
                    ...
                    # Department filter dropdown (searchable)
                    def update_dept_filter(e):
                        table_state["department"] = e.value
                        table_state["page"] = 1
                        table_content.refresh()
                    dept_filter_sel = search_select(
                        options=dept_options, 
                        value=table_state["department"], 
                        on_change=update_dept_filter,
                        label="Department",
                    ).props('dense outlined options-dense').style("min-width: 160px;")

                    # Schedule filter dropdown (searchable)
                    def update_sched_filter(e):
                        table_state["schedule"] = e.value
                        table_state["page"] = 1
                        table_content.refresh()
                    sched_filter_sel = search_select(
                        options=sched_options, 
                        value=table_state["schedule"], 
                        on_change=update_sched_filter,
                        label="Schedule",
                ...
                with ui.element("div").style("display: flex; align-items: center; gap: 8px; flex-wrap: wrap;"):
                    def update_search(e):
                        ...
                        if table_state["search"] != val:
                            ...
                            table_state["page"] = 1
                            table_content.refresh()
                    search_inp = ui.input(placeholder="Search employees...", value=table_state["search"], on_change=update_search).props('dense outlined clearable').style("width: 220px;")

                    # Reset filters button
                    def reset_filters():
                        table_state["search"] = ""
                        table_state["company"] = 0
                        table_state["department"] = "all"
                        table_state["schedule"] = "all"
                        table_state["page"] = 1
                        search_inp.value = ""
                        co_filter_sel.value = 0
                        dept_filter_sel.value = "all"
                        sched_filter_sel.value = "all"
                        table_content.refresh()
            ...
            with ui.element("div").classes("card-body").style("padding: 0; overflow-x: auto;"):
                ...
                def table_content():
                    ...
                    sel_company = table_state["company"]
                    sel_dept = table_state["department"]
                    sel_sched = table_state["schedule"]
                    ...
                    if term:
                        filtered_emps = [
                            ...
                            if term in e['emp_id'].lower() 
                            or term in e['name'].lower() 
                            or term in e['department'].lower() 
                            or term in e['position'].lower() 
                            or term in e['company'].lower()
                        ]
                    # 2. Company filter
                    if sel_company and sel_company != 0:
                        filtered_emps = [e for e in filtered_emps if e['company_id'] == sel_company]
                    # 3. Department filter
                    if sel_dept and sel_dept != "all":
                        filtered_emps = [e for e in filtered_emps if e['department'] == sel_dept]
                    # 4. Schedule filter
                    if sel_sched and sel_sched != "all":
                        filtered_emps = [e for e in filtered_emps if e['schedule_type'] == sel_sched]
                    ...
                    if not filtered_emps:
                        ...
                        <div class="empty-state">
                          ...
                          <div class="empty-state-title">No employees match your filters</div>
                                ...
                                for e in paged_emps:
                                    with ui.element("tr"):
                                        with ui.element("td").style("text-align: center;"):
                                            cb = ui.checkbox(value=e["id"] in selected_emps, on_change=lambda ev, eid=e["id"]: toggle_emp(eid, ev.value))
                                        ...
                                        with ui.element("td"):
                                            ui.html(f"<span class='badge badge-info'>{e['schedule_type']}</span>")
                                        with ui.element("td"):
                                            with ui.element("div").style("display:flex; gap: 8px; align-items: center;"):
                                                ui.button(
                                                    icon=IC.EDIT, 
                                                    on_click=lambda e=e: open_edit_dialog(e, handle_success)
                                                ).classes("action-btn-edit").props('flat round dense').tooltip("Edit Employee")
                                                ui.button(
                                                    icon=IC.DELETE, 
                                                    on_click=lambda e=e: confirm_delete_emp([e["id"]], e["name"], handle_success)
                                                ).classes("action-btn-delete").props('flat round dense').tooltip("Delete Employee")
                    ...
                    with ui.element("div").style("padding: 16px 24px; display: flex; justify-content: space-between; align-items: center; border-top: 1px solid var(--border); flex-wrap: wrap; gap: 16px;"):
                        ...
                        filtered_text = f" (filtered from {len(emps)} total)" if total_items != len(emps) else ""
